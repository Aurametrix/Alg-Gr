...
// Matcher coerces to a boolean by calling its find method
...
// make the rest of the words lowercase
assert "This Is A Test Sentence!" ==
    ("it is a VERY beautiful day!".replaceAll(/\w+/,
        { it[0].toUpperCase() + ((it.size() > 1) ? it[1..-1].toLowerCase() : '') }))

// Capitalize every word at the beginning of each line
def before='''
apple
orange
y
